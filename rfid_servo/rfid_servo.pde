...
#define SERVO_ENABLE 7        // enable servo motor
  ...
  if (isCodeValid()) {
    ...
    if (unlockDoorCheck) {
      if (currentKeyInMemory()) {
        Serial.println("Door unlocked! Move servo motor!");
        servoMotor.attach(SERVO_ENABLE);
        servoMotor.write(0);          // you will have to adjust this value (0-179) 
